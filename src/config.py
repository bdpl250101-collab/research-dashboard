...
COMPANIES: list[dict] = [
    ...
    # 그룹 전체(SK이노베이션·SK온·SK하이닉스 …). 'SK' 는 두 글자 라틴 별칭이라
    # desk/risk 같은 영단어에 걸릴 수 있어 text.alias_matches 로 대조한다.
    {"key": "sk_group", "label": "SK그룹",
     "aliases": ["SK", "에스케이"]},
]
...
# ---------------------------------------------------------------------------
# 3-4. 직무 분류
#      '어떤 직무가 올라왔는지' 를 한눈에 보기 위한 자체 분류다.
#      사람인이 주는 job-code 는 표준 분류라 화학공학 관점과 어긋날 때가 있어
#      (예: 시운전이 '생산/제조' 로 뭉뚱그려짐), 공고 제목과 사람인 직무명을
#      함께 보고 아래 버킷으로 다시 나눈다.
#      terms 는 대소문자 무시 부분 문자열. 위에서부터 먼저 걸리는 것이 우선이다.
# ---------------------------------------------------------------------------
JOB_ROLES: list[dict] = [
    {"key": "plant", "label": "플랜트·시운전",
     "terms": ["시운전", "커미셔닝", "commissioning", "플랜트", "plant",
               "배관", "기전", "준공"]},
    {"key": "rnd", "label": "연구개발",
     "terms": ["연구", "r&d", "rnd", "개발", "선행", "소재개발", "촉매",
               "배터리 개발", "research"]},
    {"key": "process", "label": "생산·공정",
     "terms": ["공정", "생산", "제조", "오퍼레이터", "운전", "process",
               "production", "조업"]},
    {"key": "quality", "label": "품질·분석",
     "terms": ["품질", "분석", "검사", "qa", "qc", "시험", "평가", "quality"]},
    {"key": "equipment", "label": "설비·정비",
     "terms": ["설비", "정비", "보전", "유틸리티", "기계", "전기", "계장",
               "maintenance"]},
    {"key": "safety", "label": "환경·안전",
     "terms": ["환경", "안전", "보건", "ehs", "hse", "소방", "탄소", "esg"]},
    {"key": "sales", "label": "영업·구매",
     "terms": ["영업", "구매", "조달", "물류", "sourcing", "sales", "마케팅"]},
    {"key": "it", "label": "IT·데이터",
     "terms": ["개발자", "소프트웨어", "데이터", "ai", "dx", "it", "보안",
               "인프라", "서버"]},
    {"key": "support", "label": "경영지원",
     "terms": ["인사", "재무", "회계", "총무", "법무", "기획", "홍보", "교육"]},
]
...
NEWS_EXCLUDE_TITLE_TERMS: list[str] = [
    ...
    # 스포츠단 기사. 대기업은 구단을 보유해 그룹명으로 검색하면 딸려 온다
    # (실측: SK그룹 -> 축구단 '제주SK' 경기 기사).
    # '라인업'·'감독' 은 사업 기사에도 쓰여서(제품 라인업 등) 넣지 않았다.
    "축구", "K리그", "프로야구", "KBO", "농구", "KBL", "배구", "구단",
    "선수단", "원정경기", "코칭스태프", "제주SK", "와이번스", "라이온즈",
]
